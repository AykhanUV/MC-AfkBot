...
function updateBotStatus(status) {
    ...
    document.getElementById('botPing').textContent = status.ping != null ? status.ping + ' ms' : '-';

    if (status.position) {
        var x = Math.round(status.position.x * 10) / 10;
        var y = Math.round(status.position.y * 10) / 10;
        var z = Math.round(status.position.z * 10) / 10;
        document.getElementById('botPosition').textContent = x + ', ' + y + ', ' + z;
    } else {
    ...
    if (status.experience) {
        document.getElementById('botExperience').textContent = 'Level ' + (status.experience.level || 0);
    } else {
    ...
    document.getElementById('healthText').textContent = status.health != null ? Math.round(health) + '/20' : '-';
    ...
    document.getElementById('foodText').textContent = status.food != null ? Math.round(food) + '/20' : '-';

    // Mining status
    var miningEl = document.getElementById('miningStatus');
    if (status.connected) {
        var autoStr = status.isMiningEnabled ? '\u2705 Auto-Mining ON' : '\u274C Auto-Mining OFF';
...
function updateConnectionUI(connected) {
    ...
    if (usernameInput) {
        ...
        document.getElementById('usernameHint').style.display = connected ? 'block' : 'none';
    }
    if (serverIp) {
        serverIp.disabled = connected;
        document.getElementById('serverIpHint').style.display = connected ? 'block' : 'none';
    }
    if (serverPort) {
        serverPort.disabled = connected;
        document.getElementById('serverPortHint').style.display = connected ? 'block' : 'none';
    }
}

function updateServerStatus(status) {
    var config = currentConfig;
    document.getElementById('serverAddress').textContent = config.server ? config.server.ip + ':' + config.server.port : '-';
    document.getElementById('serverVersion').textContent = status.version || '-';
    document.getElementById('serverPlayers').textContent = status.error ? 'Offline' : (status.online + '/' + status.max);
    document.getElementById('serverMotd').textContent = status.motd || '-';
    document.getElementById('serverLatency').textContent = status.latency ? status.latency + ' ms' : '-';
...
function updateInventory(items) {
    ...
    items.forEach(function (item) {
        ...
        el.appendChild(document.createTextNode(' ' + (item.displayName || item.name)));
        grid.appendChild(el);
    });
}
...
function populateSettingsForm(config) {
    // Bot Account
    setValue('cfg-username', config['bot-account'] ? config['bot-account'].username || '' : '');
    setValue('cfg-password', config['bot-account'] ? config['bot-account'].password || '' : '');
    setSelect('cfg-authtype', config['bot-account'] ? config['bot-account'].type || 'offline' : 'offline');

    // Server
    setValue('cfg-server-ip', config.server ? config.server.ip || '' : '');
    setValue('cfg-server-port', config.server ? config.server.port || 25565 : 25565);
    setValue('cfg-server-version', config.server ? config.server.version || '' : '');
    ...
    setValue('cfg-playeractivity-interval', playerActivity.checkIntervalSeconds || 30);
    setChecked('cfg-playeractivity-leave', playerActivity.leaveWhenPlayerJoins || false);
...
function saveSettings() {
    ...
    var chatMessages = Array.from(chatMsgInputs).map(function (el) {
    ...
    var blockTypes = blockTypesStr.split(',').map(function (s) {
        return s.trim();
    }).filter(function (s) {
        return s.length > 0;
    });
    ...
    var config = {
        ...
        utils: {
            ...
            'chat-messages': {
                ...
                'repeat-delay': parseInt(document.getElementById('cfg-chat-delay').value) || 60,
                messages: chatMessages
            },
            'auto-reconnect': document.getElementById('cfg-autoreconnect').checked,
            'auto-reconnect-delay': parseInt(document.getElementById('cfg-autoreconnect-delay').value) || 10,
            'player-activity': {
                enabled: document.getElementById('cfg-playeractivity-enabled').checked,
                checkIntervalSeconds: parseInt(document.getElementById('cfg-playeractivity-interval').value) || 30,
                leaveWhenPlayerJoins: document.getElementById('cfg-playeractivity-leave').checked
            }
        },
        mining: {
            blockTypes: blockTypes.length > 0 ? blockTypes : ['dirt', 'cobblestone'],
            interval: parseInt(document.getElementById('cfg-mining-interval').value) || 10000,
            maxDistance: parseInt(document.getElementById('cfg-mining-distance').value) || 5
        },
