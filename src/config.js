...
class ConfigManager {
    ...
    load() {
        try {
            if (fs.existsSync(SETTINGS_PATH)) {
                ...
                console.log('[Config] settings.json not found, creating with defaults.');
    ...
    save() {
        try {
            fs.writeFileSync(SETTINGS_PATH, JSON.stringify(this.config, null, 2), 'utf-8');
        } catch (err) {
    ...
    loadRuntimeState() {
        ...
        try {
            if (fs.existsSync(RUNTIME_STATE_PATH)) {
                ...
                return { isMiningEnabled: typeof state.isMiningEnabled === 'boolean' ? state.isMiningEnabled : true };
            }
        } catch (err) {
        ...
        // Write default
        try {
            fs.writeFileSync(RUNTIME_STATE_PATH, JSON.stringify(defaultState, null, 2));
        } catch (e) { /* ignore */ }
        return defaultState;
    }
